...
def _main(args):
    ...
    if 1:
        train(
            ...
            detectors_mask,
            matching_true_boxes,
            weights_file='YOLO_fine_tuned.h5'
        )
...
def get_detector_mask(boxes, anchors):
    ...
    for i, box in enumerate(boxes):
        detectors_mask[i], matching_true_boxes[i] = preprocess_true_boxes(box, anchors, [IMAGE_SIZE[1], IMAGE_SIZE[0]])
...
def create_model(anchors, class_names, load_pretrained=True, freeze_body=True):
    ...
    detectors_mask_shape = (IMAGE_SIZE[1]//32, IMAGE_SIZE[0]//32, num_anchors, 1)
    matching_boxes_shape = (IMAGE_SIZE[1]//32, IMAGE_SIZE[0]//32, num_anchors, 5)
...
def train(model, class_names, anchors, image_data, boxes, detectors_mask, matching_true_boxes, validation_split=0.1, weights_file=None):
    '''
